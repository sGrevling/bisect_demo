...
- ```git bisect good ###``` with ### as a commit where the bug did not exist (e.g. ```28dd2c25```, v1 of the app)
...
This project contains a test called ```fantastic_modal_works``` so after the bisect process is started and the scope defined we can run: 

```git bisect run jest -t  "fantastic_modal_works"```
